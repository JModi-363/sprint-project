...
def init_db():
    ...
    if count == 0:
        # Seed default menu items with descriptions and sustainability notes
        default_items = [
            # Paint bases
            (
                "paint_base",
            ...
            (
                ...
                "Contains natural oils; requires solvents which may impact air quality",
                "",
            ),
            (
                "paint_base",
            ...
            (
                ...
                "Egg-based historical formula; biodegradable and low-impact",
                "",
            ),
            (
                "paint_base",
                ...
                "Water-based and non-toxic; environmentally safe",
                "",
            ),
            ...
            # Additives
            ("additives", "Thickener", 0, 0, "", "Non-toxic, biodegradable polymer", ""),
            ("additives", "Antioxidant", 0, 0, "", "Helps extend paint life; low environmental impact", ""),
            ("additives", "Hardener", 0, 0, "", "Solvent-based; moderate environmental impact", ""),
            ("additives", "Extender", 0, 0, "", "Increases coverage; reduces total paint waste", ""),
            ("additives", "None", 0, 0, "", "No additives used", ""),
        ]
...
if st.session_state.artist is None:
    ...
    if action == "Place Order":
        ...
        # Main order form: only inputs + submit
        ...
        # Live metadata display (updates immediately when dropdowns change)
        meta_base = menu.get_metadata("paint_base", paint_base)
        if meta_base:
            st.info(
                f"**Paint Base Description:** {meta_base['description']}\n\n"
        ...
        if meta_add:
            st.info(
                f"**Additive Description:** {meta_add['description']}\n\n"
                f"**Sustainability:** {meta_add['sustainability_info']}"
            )

        # Show additive parts cost hint live
